...
typed todos, auth recipe, and CloudKitDemo/counters example leaf dispatch
without executable reparsing, init scaffolding, schema/perms generate/verify,
the todo-links, reminders, and sync-ups example leaf arguments with malformed
no-bootstrap coverage, auth show/guest/token/id-token/OAuth/OAuth
