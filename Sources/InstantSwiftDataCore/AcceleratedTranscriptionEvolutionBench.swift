...
///
/// The two-hour profile deliberately rewrites one bounded open segment instead
...
public struct AcceleratedTranscriptionEvolutionProfile: Codable, Equatable, Sendable {
  ...
  public init(
    ...
    sampleEveryRevisionCount: Int = 100
  ) {
    self.name = name
...
public enum AcceleratedTranscriptionEvolutionBench {
  ...
  public static func runLocalFirst(
    ...
    for segmentIndex in 0..<profile.segmentCount {
      for revision in 1...profile.revisionsPerSegment {
          ...
          || completedRevisionCount == profile.revisionCount
        {
          ...
          let pending = await runtime.pendingMutations().count
    ...
    let actualHash = canonicalHash(actualPayloads.map(canonicalFinalLine(payload:)))
    let finalWordCount = actualPayloads.reduce(0) { $0 + $1.words.count }

    _ = try? await runtime.closeConnection()
    _ = try await runtime.connect()
    _ = try await runtime.flushPendingMutations()
    let finalPendingMutationCount = await runtime.pendingMutations().count
    ...
    let averageCPU = wallSeconds > 0 ? (userSeconds + systemSeconds) / wallSeconds * 100 : 0
    ...
    let effectiveAcceleration = wallSeconds > 0
      ? Double(profile.logicalDurationSeconds) / wallSeconds
      : .infinity
    let throughput = wallSeconds > 0 ? Double(profile.revisionCount) / wallSeconds : .infinity
    ...
    let incrementalPeak = signedDifference(peakFootprint, baselineMemory.physical)
    let settledGrowth = signedDifference(endMemory.physical, baselineMemory.physical)
    return AcceleratedTranscriptionEvolutionResult(
      ...
      memory: AcceleratedMemoryUsage(
        ...
        incrementalPeakPhysicalFootprintBytes: incrementalPeak,
        settledPhysicalFootprintGrowthBytes: settledGrowth,
