...
public struct CloudKitDemoV3AppConfiguration: Hashable, Sendable {
  ...
  public static func environment(
    ...
    return Self(
      appID: configuredAppID.flatMap { $0.isEmpty ? nil : $0 } ?? "cloudkit-demo-v3-local",
  ...
  @MainActor
  public final class CloudKitDemoV3BootstrapModel: ObservableObject {
    @Published public private(set) var client: InstantSwiftDataClient?
    @Published public private(set) var errorMessage: String?

    public let configuration: CloudKitDemoV3AppConfiguration
    private var task: Task<Void, Never>?

    public init(configuration: CloudKitDemoV3AppConfiguration) {
  ...
  @MainActor
  public struct CloudKitDemoV3BootstrapScreen: View {
    @StateObject private var model: CloudKitDemoV3BootstrapModel

    public init(model: CloudKitDemoV3BootstrapModel) {
    ...
    public var body: some View {
      Group {
        if model.client != nil {
          CloudKitDemoV3Screen()
        } else if let errorMessage = model.errorMessage {
          Text(errorMessage)
        } else {
          ProgressView("Opening Shared Counters")
        }
      }
