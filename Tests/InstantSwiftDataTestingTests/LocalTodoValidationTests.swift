...
struct LocalTodoValidationTests {
  ...
  func parityCoverageValidationHarnessSummarizesIncompleteCoverage() throws {
    ...
    expectNoDifference(run.result.recordCount, 57)
    expectNoDifference(run.result.exactCount, 11)
    expectNoDifference(run.result.adaptedCount, 43)
    ...
    expectNoDifference(run.summary.events, Array(repeating: "parity-record", count: 57))
