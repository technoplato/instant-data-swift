...
struct DualResidencyFootprintMeasureTests {
  ...
  func publishGatePendingAndColdReopenMetrics() async throws {
    ...
    let mode = ProcessInfo.processInfo.environment["INSTANT_MEASURE_MODE"] ?? "both"
    let envPath = ProcessInfo.processInfo.environment["INSTANT_MEASURE_DB"]
    let cacheURL: URL
    if let envPath, !envPath.isEmpty {
      cacheURL = URL(fileURLWithPath: envPath)
    ...
    if mode == "reopen" || mode == "both" {
      ...
      print(
        "AUTORESEARCH_METRIC profile=publishGate mode=cold-reopen peak_physical_growth_bytes=\(coldGrowth) mb_growth=\(String(format: "%.1f", Double(coldGrowth)/1024/1024)) after_abs=\(coldAfter?.physicalFootprintBytes ?? 0)"
      )
      #expect(coldGrowth > 0)
    }
  }
