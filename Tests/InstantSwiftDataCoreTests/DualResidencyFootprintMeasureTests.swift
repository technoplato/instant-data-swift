...
struct DualResidencyFootprintMeasureTests {
  @Test
  func scribeShapedSeedReportsFootprintForAutoresearch() async throws {
    // Full publish-gate graph: ~80×120 words — same class as LinkedInfinite soak.
    ...
    // Encourage release of transient seed/query buffers before sampling.
    for _ in 0..<5 {
      autoreleasepool { _ = [UInt8](repeating: 0, count: 1) }
    }
    ...
    print(
      "AUTORESEARCH_METRIC profile=publishGate peak_physical_growth_bytes=\(growth) baseline=\(baseline?.physicalFootprintBytes ?? 0) after=\(after?.physicalFootprintBytes ?? 0) mb_growth=\(String(format: "%.1f", Double(growth)/1024/1024))"
    )
    #expect(after != nil)
    #expect(growth > 0)
  }
