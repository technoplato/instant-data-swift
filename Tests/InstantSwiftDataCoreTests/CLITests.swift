...
extension InstantStoreTests {
  ...
  @Test
  func cliGeneratesAndVerifiesExactTypingIndicatorContract() throws {
    let homeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("InstantSwiftDataCLITests-\(UUID().uuidString)", isDirectory: true)
    let schemaURL = homeURL.appendingPathComponent("typing-indicator.schema.ts")
    let permissionsURL = homeURL.appendingPathComponent("typing-indicator.perms.ts")
    try FileManager.default.createDirectory(at: homeURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: homeURL) }
    ...
    let generatedSchema = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "schema", "generate", "--example", "typing-indicator",
            "--to", schemaURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(generatedSchema.example, "typing-indicator")
    let schemaSource = try String(contentsOf: schemaURL, encoding: .utf8)
    #expect(schemaSource.contains("\"typing-indicator-example\""))
    #expect(schemaSource.contains("\"chat-input\": i.boolean().optional()"))
    #expect(!schemaSource.contains("displayName"))
    ...
    let schemaVerify = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "schema", "verify", "--example", "typing-indicator",
            "--from", schemaURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(schemaVerify.example, "typing-indicator")
    ...
    let generatedPermissions = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "perms", "generate", "--example", "typing-indicator",
            "--to", permissionsURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(generatedPermissions.example, "typing-indicator")
    ...
    let permissionsVerify = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "perms", "verify", "--example", "typing-indicator",
            "--from", permissionsURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(permissionsVerify.example, "typing-indicator")
  ...
  @Test
  func cliRoundTripsCanonicalCustomCursorsArtifacts() throws {
    let homeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("InstantSwiftDataCLITests-\(UUID().uuidString)", isDirectory: true)
    let schemaURL = homeURL.appendingPathComponent("custom-cursors.schema.ts")
    let permissionsURL = homeURL.appendingPathComponent("custom-cursors.perms.ts")
    try FileManager.default.createDirectory(at: homeURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: homeURL) }
    ...
    let generatedSchema = try JSONDecoder().decode(
      CLIGeneratedArtifactOutput.self,
      from: Data(try runCLI([
        "schema", "generate", "--example", "custom-cursors",
        "--to", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(generatedSchema.example, "custom-cursors")
    let schemaSource = try String(contentsOf: schemaURL, encoding: .utf8)
    #expect(schemaSource.contains("\"cursors-example\""))
    #expect(
      schemaSource.contains(
    ...
    let schemaVerify = try JSONDecoder().decode(
      CLISchemaVerifyOutput.self,
      from: Data(try runCLI([
        "schema", "verify", "--example", "custom-cursors",
        "--from", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(schemaVerify.example, "custom-cursors")
    expectNoDifference(schemaVerify.entityCount, 0)
    expectNoDifference(schemaVerify.linkCount, 0)
    expectNoDifference(schemaVerify.warnings, [])

    _ = try runCLI([
      "perms", "generate", "--example", "custom-cursors",
      "--to", permissionsURL.path, "--json",
    ], homeURL: homeURL)
    let permissionsVerify = try JSONDecoder().decode(
      CLIPermissionsVerifyOutput.self,
      from: Data(try runCLI([
        "perms", "verify", "--example", "custom-cursors",
        "--from", permissionsURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(permissionsVerify.example, "custom-cursors")
    expectNoDifference(permissionsVerify.namespaceCount, 0)
    expectNoDifference(permissionsVerify.allowRuleCount, 0)
  }
  ...
  @Test
  func cliRoundTripsCanonicalStroopwafelArtifacts() throws {
    let homeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("InstantSwiftDataCLITests-\(UUID().uuidString)", isDirectory: true)
    let schemaURL = homeURL.appendingPathComponent("stroopwafel.schema.ts")
    let permissionsURL = homeURL.appendingPathComponent("stroopwafel.perms.ts")
    try FileManager.default.createDirectory(at: homeURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: homeURL) }
    ...
    let generatedSchema = try JSONDecoder().decode(
      CLIGeneratedArtifactOutput.self,
      from: Data(try runCLI([
        "schema", "generate", "--example", "stroopwafel",
        "--to", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(generatedSchema.example, "stroopwafel")
    let schemaSource = try String(contentsOf: schemaURL, encoding: .utf8)
    #expect(schemaSource.contains("\"$users\": i.entity"))
    #expect(schemaSource.contains("readyIds: i.json()"))
    #expect(schemaSource.contains("gamePoints:"))
    ...
    let schemaVerify = try JSONDecoder().decode(
      CLISchemaVerifyOutput.self,
      from: Data(try runCLI([
        "schema", "verify", "--example", "stroopwafel",
        "--from", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(schemaVerify.example, "stroopwafel")
    expectNoDifference(schemaVerify.entityCount, 4)
    expectNoDifference(schemaVerify.linkCount, 4)
    expectNoDifference(schemaVerify.warnings, [])

    _ = try runCLI([
      "perms", "generate", "--example", "stroopwafel",
      "--to", permissionsURL.path, "--json",
    ], homeURL: homeURL)
    let permissionsVerify = try JSONDecoder().decode(
      CLIPermissionsVerifyOutput.self,
      from: Data(try runCLI([
        "perms", "verify", "--example", "stroopwafel",
        "--from", permissionsURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(permissionsVerify.example, "stroopwafel")
    expectNoDifference(permissionsVerify.namespaceCount, 4)
    expectNoDifference(permissionsVerify.allowRuleCount, 17)
  }

  @Test
  func cliRoundTripsRemindersV3Artifacts() throws {
    let homeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("InstantSwiftDataCLITests-\(UUID().uuidString)", isDirectory: true)
    let schemaURL = homeURL.appendingPathComponent("reminders.schema.ts")
    let permissionsURL = homeURL.appendingPathComponent("reminders.perms.ts")
    try FileManager.default.createDirectory(at: homeURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: homeURL) }
    ...
    let generatedSchema = try JSONDecoder().decode(
      CLIGeneratedArtifactOutput.self,
      from: Data(try runCLI([
        "schema", "generate", "--example", "reminders",
        "--to", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(generatedSchema.example, "reminders")
    let schemaSource = try String(contentsOf: schemaURL, encoding: .utf8)
    #expect(schemaSource.contains("remindersLists: i.entity"))
    #expect(schemaSource.contains("reminders: i.entity"))
    #expect(schemaSource.contains("tags: i.entity"))
    #expect(schemaSource.contains("remindersList:"))
    #expect(schemaSource.contains("remindersListsReaders:"))
    #expect(schemaSource.contains("v3_sharesRoot:"))

    let schemaVerify = try JSONDecoder().decode(
      CLISchemaVerifyOutput.self,
      from: Data(try runCLI([
        "schema", "verify", "--example", "reminders",
        "--from", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(schemaVerify.example, "reminders")
    expectNoDifference(schemaVerify.entityCount, 6)
    expectNoDifference(schemaVerify.linkCount, 9)
    expectNoDifference(schemaVerify.warnings, [])

    _ = try runCLI([
      "perms", "generate", "--example", "reminders",
      "--to", permissionsURL.path, "--json",
    ], homeURL: homeURL)
    let permissionsVerify = try JSONDecoder().decode(
      CLIPermissionsVerifyOutput.self,
      from: Data(try runCLI([
        "perms", "verify", "--example", "reminders",
        "--from", permissionsURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(permissionsVerify.example, "reminders")
    expectNoDifference(permissionsVerify.namespaceCount, 6)
    expectNoDifference(permissionsVerify.allowRuleCount, 21)
  }
  ...
  @Test
  func cliRoundTripsAppBuilderV3Artifacts() throws {
    let homeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("InstantSwiftDataCLITests-\(UUID().uuidString)", isDirectory: true)
    let schemaURL = homeURL.appendingPathComponent("app-builder.schema.ts")
    let permissionsURL = homeURL.appendingPathComponent("app-builder.perms.ts")
    try FileManager.default.createDirectory(at: homeURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: homeURL) }
    ...
    let generatedSchema = try JSONDecoder().decode(
      CLIGeneratedArtifactOutput.self,
      from: Data(try runCLI([
        "schema", "generate", "--example", "app-builder",
        "--to", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(generatedSchema.example, "app-builder")
    let schemaSource = try String(contentsOf: schemaURL, encoding: .utf8)
    #expect(schemaSource.contains("\"$files\": i.entity"))
    #expect(schemaSource.contains("\"$users\": i.entity"))
    #expect(schemaSource.contains("builds: i.entity"))
    ...
    let schemaVerify = try JSONDecoder().decode(
      CLISchemaVerifyOutput.self,
      from: Data(try runCLI([
        "schema", "verify", "--example", "app-builder",
        "--from", schemaURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(schemaVerify.example, "app-builder")
    expectNoDifference(schemaVerify.entityCount, 3)
    expectNoDifference(schemaVerify.attributeCount, 13)
    expectNoDifference(schemaVerify.linkCount, 2)
    expectNoDifference(schemaVerify.warnings, [])

    _ = try runCLI([
      "perms", "generate", "--example", "app-builder",
      "--to", permissionsURL.path, "--json",
    ], homeURL: homeURL)
    let permissionsVerify = try JSONDecoder().decode(
      CLIPermissionsVerifyOutput.self,
      from: Data(try runCLI([
        "perms", "verify", "--example", "app-builder",
        "--from", permissionsURL.path, "--json",
      ], homeURL: homeURL).utf8)
    )
    expectNoDifference(permissionsVerify.example, "app-builder")
    expectNoDifference(permissionsVerify.namespaceCount, 1)
    expectNoDifference(permissionsVerify.allowRuleCount, 5)
  }
  ...
  @Test
  func cliGeneratesAndVerifiesVoiceTrailContract() throws {
    let homeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("InstantSwiftDataCLITests-\(UUID().uuidString)", isDirectory: true)
    let schemaURL = homeURL.appendingPathComponent("voice-trail.schema.ts")
    let permissionsURL = homeURL.appendingPathComponent("voice-trail.perms.ts")
    try FileManager.default.createDirectory(at: homeURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: homeURL) }
    ...
    let generatedSchema = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "schema", "generate", "--example", "voice-trail",
            "--to", schemaURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(generatedSchema.example, "voice-trail")
    let schemaSource = try String(contentsOf: schemaURL, encoding: .utf8)
    #expect(schemaSource.contains("v3_capture_recordingsReaders"))
    #expect(schemaSource.contains("v3_capture_recordingsWriters"))
    #expect(schemaSource.contains("v3_sharesRoot"))
    ...
    let schemaVerify = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "schema", "verify", "--example", "voice-trail",
            "--from", schemaURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(schemaVerify.example, "voice-trail")
    expectNoDifference(schemaVerify.entityCount, 6)
    expectNoDifference(schemaVerify.linkCount, 9)
    expectNoDifference(schemaVerify.warnings, [])
    ...
    let generatedPermissions = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "perms", "generate", "--example", "voice-trail",
            "--to", permissionsURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(generatedPermissions.example, "voice-trail")
    let permissionsSource = try String(contentsOf: permissionsURL, encoding: .utf8)
    #expect(permissionsSource.contains("auth.id in data.ref('recording.writers.id')"))
    #expect(permissionsSource.contains("auth.id in data.ref('share.owner.id')"))
    ...
    let permissionsVerify = try JSONDecoder().decode(
      ...
      from: Data(
        try runCLI(
          [
            "perms", "verify", "--example", "voice-trail",
            "--from", permissionsURL.path,
            "--json",
          ],
          homeURL: homeURL
    ...
    expectNoDifference(permissionsVerify.example, "voice-trail")
    expectNoDifference(permissionsVerify.namespaceCount, 6)
    expectNoDifference(permissionsVerify.allowRuleCount, 21)
  }
  ...
  func cliValidationParityReportEmitsCoverageProvenance() throws {
    ...
    for id in [
      "instant.svelte.use-auth-starts-loading",
      "instant.svelte.use-auth-updates-state",
      "instant.svelte.use-auth-cached-state",
      "instant.svelte.use-auth-error",
      "instant.vue.use-auth-starts-loading",
      "instant.vue.use-auth-updates-state",
      "instant.vue.use-auth-cached-state",
      "instant.vue.use-auth-error",
      "instant.vue.use-auth-unsubscribes-on-scope-stop",
    ] {
      #expect(
        jsonOutput.records.contains { $0.id == id && $0.status == "adapted" },
    ...
    for expected in [
      ("instant.persisted-object.saves-values", "adapted"),
      ("instant.persisted-object.merges-existing-values", "adapted"),
      ("instant.persisted-object.load-notification", "not-applicable"),
      ...
      #expect(
        jsonOutput.records.contains { $0.id == expected.0 && $0.status == expected.1 },
        "Expected \(expected.1) PersistedObject parity record \(expected.0)"
      )
    }
    #expect(
      jsonOutput.records.contains {
    ...
    #expect(
      jsonOutput.records.contains {
        $0.id == "sqlite.draft.relation-ref-save" && $0.status == "adapted"
      }
    )
    #expect(
      jsonOutput.records.contains {
