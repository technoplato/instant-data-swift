...
for example in validation recording-action sharing voice-trail reminders; do
...
swift run --package-path "${ROOT}" instant-swift-data schema verify \
  --example sharing \
  --from "${ROOT}/validation/fixtures/sharing.schema.ts" \
  --json >/dev/null
...
printf '{"case":"validation.typescript.generated-contract","event":"typecheck","ok":true,"details":{"upstreamRevision":"%s","coreVersion":"%s","adminVersion":"%s","typescriptVersion":"%s","compilerWarningCount":0,"artifacts":{"validationSchemaSHA256":"%s","validationPermissionsSHA256":"%s","recordingActionSchemaSHA256":"%s","recordingActionPermissionsSHA256":"%s","sharingSchemaSHA256":"%s","sharingPermissionsSHA256":"%s","voiceTrailSchemaSHA256":"%s","voiceTrailPermissionsSHA256":"%s","remindersSchemaSHA256":"%s","remindersPermissionsSHA256":"%s","serverRecordingActionSchemaSHA256":"%s","serverRecordingActionPermissionsSHA256":"%s"}}}\n' \
  ...
  "$(sha256 "${GENERATED}/reminders.schema.ts")" \
  "$(sha256 "${GENERATED}/reminders.perms.ts")" \
