...
RESULTS_DIR="${INSTANT_SWIFT_DATA_STREAMS_V3_RESULTS_DIR:-/tmp/instant-data-swift-streams-v3-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ -n "$(git -C "${ROOT}" status --porcelain)" ]]; then
  ...
  if [[ "${INSTANT_SWIFT_DATA_ALLOW_DIRTY_CONTRACT_RUN:-0}" != "1" ]]; then
    echo "Streams V3 verification requires a clean worktree." >&2
    exit 1
  fi
fi
...
swift run --package-path "${ROOT}" instant-swift-data schema generate \
  --example streams --to "${PUSH_DIR}/instant.schema.ts" --json \
...
swift run --package-path "${ROOT}" instant-swift-data schema verify \
  --example streams --from "${PULL_DIR}/instant.schema.ts" --json \
  >"${RESULTS_DIR}/swift-server-schema-verify.json"
swift run --package-path "${ROOT}" instant-swift-data perms verify \
  --example streams --from "${PULL_DIR}/instant.perms.ts" --json \
...
corepack pnpm --dir "${RUNNER}" exec tsx src/streams-v3-live-contract.ts \
  >"${RESULTS_DIR}/streams-v3.json"
...
const live = readJSON("streams-v3.json");

assert.equal(live.ok, true);
assert.equal(live.details.compilerWarningCount, 0);
assert.deepStrictEqual(live.details.warnings, []);
assert.equal(live.details.typeScriptWriter.content, "typescript to swift 🚀");
assert.equal(live.details.typeScriptWriter.byteCount, 24);
assert.equal(live.details.swiftWriter.content, "swift to typescript 🚀");
assert.equal(live.details.swiftWriter.byteCount, 24);
assert.equal(live.details.swift.typeScriptContent, live.details.typeScriptWriter.content);
assert.equal(live.details.swift.swiftContent, live.details.swiftWriter.content);
...
assert.equal(permissions.allowRuleCount, 2);
assert.equal(permissions.rateLimitCount, 0);

const evidence = {
  case: "validation.streams-v3-app.live-contract",
  ...
  details: {
    ...
    streams: live.details,
  },
};
process.stdout.write(`${JSON.stringify(evidence, null, 2)}\n`);
NODE

echo "Streams V3 evidence: ${RESULTS_DIR}/evidence.json"
