...
RESULTS="${INSTANT_SWIFT_DATA_CUSTOM_CURSORS_RESULTS_DIR:-/tmp/instant-data-swift-custom-cursors-v3-$(date -u +%Y%m%dT%H%M%SZ)}"
PUSH="${RESULTS}/push"
PULL="${RESULTS}/pull"
...
if [[ -n "$(git -C "${ROOT}" status --porcelain)" ]]; then
  echo "Custom Cursors V3 verification requires a clean worktree." >&2
  exit 1
fi
if [[ ! -x "${CLI}" ]]; then
...
swift run --package-path "${ROOT}" instant-swift-data schema generate \
  --example custom-cursors --to "${PUSH}/instant.schema.ts" --json \
  >"${RESULTS}/swift-schema-generate.json"
swift run --package-path "${ROOT}" instant-swift-data perms generate \
  --example custom-cursors --to "${PUSH}/instant.perms.ts" --json \
  >"${RESULTS}/swift-perms-generate.json"

printf 'Provisioned Custom Cursors V3 app through getadb.com (%s).\n' \
...
swift run --package-path "${ROOT}" instant-swift-data schema verify \
  --example custom-cursors --from "${PULL}/instant.schema.ts" --json \
  >"${RESULTS}/swift-server-schema-verify.json"
swift run --package-path "${ROOT}" instant-swift-data perms verify \
  --example custom-cursors --from "${PULL}/instant.perms.ts" --json \
...
INSTANT_SWIFT_DATA_CUSTOM_CURSORS_SCHEMA_PATH="${PUSH}/instant.schema.ts" \
  corepack pnpm --dir "${RUNNER}" exec tsx src/custom-cursors-v3-live-contract.ts \
  >"${RESULTS}/custom-cursors-v3.json"
...
const live = read("custom-cursors-v3.json");
...
assert.equal(live.details.swift.publishedName, "swift-custom-avatar");
...
assert.equal(live.details.swift.observedName, "typescript-custom-avatar");
assert.equal(live.details.swift.remoteCursorCount, 1);
assert.equal(live.details.swift.remoteCursorCountAfterClear, 0);
assert.equal(live.details.swift.remoteNamedPeerCountAfterClear, 1);
...
const evidence = {
  case: "validation.custom-cursors-v3-app.live-contract",
  ...
  details: {
    ...
    customCursors: live.details,
  },
};
process.stdout.write(`${JSON.stringify(evidence, null, 2)}\n`);
NODE

echo "Custom Cursors V3 evidence: ${RESULTS}/evidence.json"
