...
RESULTS_DIR="${INSTANT_SWIFT_DATA_TYPING_INDICATOR_V3_RESULTS_DIR:-/tmp/instant-data-swift-typing-indicator-v3-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ -z "${APP_ID}" ]]; then
  (
    cd "${PUSH_DIR}"
    "${CLI}" init --temp --title "instant-data-swift-typing-indicator-v3" --yes --env .instant.env
...
const live = readJSON("typing-indicator-v3.json");
...
const evidence = {
  case: "validation.typing-indicator-v3-app.live-contract",
...
echo "Typing Indicator V3 evidence: ${RESULTS_DIR}/evidence.json"
